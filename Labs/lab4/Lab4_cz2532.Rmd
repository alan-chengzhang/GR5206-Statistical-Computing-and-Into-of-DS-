...
```{r}
 KNN.decision <- function(lag_new, K = 5, lag_data) {
   
   n <- dim(lag_data)[1]
   
   stopifnot(dim(lag_data)[1] == n, dim(lag_data)[2] == 5, length(lag_new) == 5, K <= n)
   
   dists <- c(length = n)
   for (i in (1:n)){
     dists[i] <- sqrt(sum((lag_data[i,]-lag_new)^2))
   }
...
for (i in (1:dim(test)[1])){
  new <- test[i,c("Lag1","Lag2","Lag3","Lag4","Lag5")]
  prediction <- KNN.decision(lag_data = train[,c("Lag1","Lag2","Lag3","Lag4","Lag5")],lag_new = new)
  result <- c(result, prediction)
}

test.error <- sum(result != test$Direction)/dim(test)[1]
test.error
...
for (i in (1:dim(test)[1])){
  new <- test[i,c("Lag1","Lag2","Lag3","Lag4","Lag5")]
  prediction_k3 <- KNN.decision(lag_data = train[,c("Lag1","Lag2","Lag3","Lag4","Lag5")],lag_new = new, K=3)
  result_k3 <- c(result_k3, prediction_k3)
}
test.error_k3 <- sum(result_k3 != test$Direction)/dim(test)[1]
test.error_k3
...
```{r}
fold_number <- rep(1:9, each = 121)
fold <- sample(fold_number,size = 1089, replace = FALSE)
...
```{r}
CV_data <- cbind(Weekly, fold)

CV_error <- c()
for (i in (1:9)){
  CV_test <- CV_data[CV_data[,"fold"] == i,]
  CV_train <- CV_data[CV_data[,"fold"] != i,]
  
  CV_result <- c()
  for (j in (1:dim(CV_test)[1])){
    CV_new <- CV_test[j,c("Lag1","Lag2","Lag3","Lag4","Lag5")]
    CV_prediction <- KNN.decision(lag_data = CV_train[,c("Lag1","Lag2","Lag3","Lag4","Lag5")],lag_new = CV_new, K=5)
    CV_result <- c(CV_result, CV_prediction)
  }
  CV_test.error <- sum(CV_result != CV_test$Direction)/dim(CV_test)[1]
  CV_error <- c(CV_error, CV_test.error)
  i
}
CV_error
...
for (k in c(1,3,7)){
  ...
  for (i in (1:9)){
    ...
    for (j in (1:121)){
      CV_new <- CV_test[j,c("Lag1","Lag2","Lag3","Lag4","Lag5")]
      CV_prediction <- KNN.decision(lag_data = CV_train[,c("Lag1","Lag2","Lag3","Lag4","Lag5")],
                                    lag_new = CV_new, K=k)
      CV_result <- c(CV_result, CV_prediction)
    }
    CV_test_error <- sum(CV_result != CV_test$Direction)/dim(CV_test)[1]
    CV_error <- c(CV_error, CV_test_error)
  }
  CV_error_mean <- mean(CV_error)
  CV_error_K <- c(CV_error_K, CV_error_mean)
  CV_error_K
}

c(mean(CV_error),CV_error_K)


```
##### The error is smallest when K=3
