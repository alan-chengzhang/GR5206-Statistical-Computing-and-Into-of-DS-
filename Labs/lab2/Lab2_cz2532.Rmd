...
```{r}
setwd("C:/Users/Alan_/Desktop/Semester1 Courses/STAT computing/Week2/lab 2")
...
lm0
```
##### The cofficients for Intercept is -2344. The cofficients for the predictor (carat) is 7848
...
```{r}
# We get the row rumbers (index) of sampling value
...
```{r}
# the original estimate of beta is the cofficient of "carat" in lm0, the estimate of bootstarp of each sample is the cofficent of "carat" in resampled_ests

diff_estimates <- c(c(coefficients(lm0))[2] - resampled_ests[,2])
length(diff_estimates)
```
##### diff_estimates is a vector with length of B = 1000
...
```{r}
hist(resampled_ests[,2], main = "Histogram of bootstrap estimates of beta1", xlab = "bootstrap estimates of beta1")
```
...
beta1<-round(lm0$coefficients,3)[2]

Ci_lower<-2*beta1 - quantile(resampled_ests[, 2], 0.975)
Ci_upper<-2*beta1 - quantile(resampled_ests[, 2], 0.025)
int <- c(Ci_lower, Ci_upper)
int

quantile(resampled_ests[, "Slope_Est"], 0.025)
