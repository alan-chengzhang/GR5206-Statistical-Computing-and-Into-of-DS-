...
##### Reason 1: Althogh the frequency will stabalize in  when N becomes big enough, the n=100 is not big enough. Since n is not big enough, the mean and standard deviation are just tends to be 0 and 1. Even though let n be 10000, the two values will still not be 0 and 1. Because the frequency of test cannot be exactly the probability and the finite random numbers can never be in the normal distribution.
##### Reason 2: From another prospective, what we do here is sampling in a big group, just like sampling 1% of the population in America. If we do it properly, the samples will represent most infornation of the whole group, but will never convey the exact same infomation of the whole group.

```{r}
# generate normal100. calculate its mean and standard deviation
normal100 <- c(rnorm(n = 100, mean = 0, sd = 1))
mean(normal100)
sd(normal100)
...
```{r}
# histgram of normal100
hist(normal100, breaks = 10, main = "normal100")
...
```{r}
# generate normal10 normal1000 normal10000 normal100000
normal10 <- c(rnorm(n = 10, mean = 0, sd = 1))
normal1000 <- c(rnorm(n = 1000, mean = 0, sd = 1))
normal10000 <- c(rnorm(n = 10000, mean = 0, sd = 1))
normal100000 <- c(rnorm(n = 100000, mean = 0, sd = 1))
...
##### Explaination: Just as I mentioned in the first question, the frequancy tends to be the frequency when the times in an experiment increase. So, when n becomes larger, the probability becomes closer to the probability and the mean of n is closer to the mean of standard normal distribution, which is 0 (but will never be exact 0 with finite number of n).

```{r}
# mean of 4 sets of random variables
...
```{r}
# Generate 1 million random draws from a normal distribution with mean=3 varience=2
...
```{r}
# mean of elements larger than 3 
mean(normal1mil_L3 <- normal1mil[which(normal1mil > 3)])
...
```{r}
# matrix 100*10000 for normal1mil
...
```{r}
# mean of 1234th column of normal1mil_mat
...
```{r}
# calculate the means of each column of normal1mil_mat
means_col <- colSums(normal1mil_mat)/length(normal1mil_mat[,1])
...
##### Explaination: This histogram approximates normal distribution with a mean of 3

```{r}
# histogram of the column means you calculated in task (9) & explain the distribution
hist(means_col, breaks = 20, main = "column means")
